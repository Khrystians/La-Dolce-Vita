...
<head>
  ...
  <title>Top Bar</title>
  <link rel="stylesheet" href="../Css/Cocina-Pendientes.css">
</head>
<body>
  <div class="top-bar">
    <!-- Logo + Nombre -->
    ...
    // Obtener mesas con pedidos pendientes
    ...
    <div class="mesa-container" id="mesa-slider">
      ...
      if ($result_mesas && $result_mesas->num_rows > 0) {
        ...
        echo "<div style='padding:1rem 2rem;color:#888;font-size:1.2rem;'>No hay mesas con pedidos pendientes.</div>";
      }
      ?>
    </div>
  ...
  <div style="display: flex; align-items: flex-start; justify-content: center;">
    ...
    <div class="container" id="pedido-carousel">
      <?php
      // --- Manejo del cambio de estado a "en cocina" ---
      if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['preparar_pedido_id'])) {
        $pedido_id = intval($_POST['preparar_pedido_id']);
        $conn = new mysqli("localhost", "root", "", "la_dolce_vita");
        if (!$conn->connect_error) {
          $conn->query("UPDATE orders SET status='en cocina' WHERE id=$pedido_id");
          $conn->close();
      ...
      if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar_pedido_id'])) {
        $pedido_id = intval($_POST['eliminar_pedido_id']);
        $conn = new mysqli("localhost", "root", "", "la_dolce_vita");
        if (!$conn->connect_error) {
          // Eliminar los platos asociados primero por integridad referencial
          $conn->query("DELETE FROM order_dishes WHERE order_id=$pedido_id");
          // Eliminar el pedido
          $conn->query("DELETE FROM orders WHERE id=$pedido_id");
          $conn->close();
          // Recargar para reflejar el cambio
          echo "<script>location.href=location.href;</script>";
          exit;
      ...
      if ($result_pedidos && $result_pedidos->num_rows > 0) {
          ...
          <div class="mesa-card">
            ...
            <div class="footer-bar">
              <form method="post" style="display:inline;">
                <input type="hidden" name="preparar_pedido_id" value="<?php echo $pedido_id; ?>">
                <button class="btn-preparar" type="submit">Preparar</button>
              </form>
              <form method="post" style="display:inline;">
                <input type="hidden" name="eliminar_pedido_id" value="<?php echo $pedido_id; ?>">
                <button class="btn-eliminar" type="submit" onclick="return confirm('¿Seguro que deseas eliminar este pedido?');">Eliminar</button>
              </form>
            </div>
          </div>
          <?php
        ...
        echo "<div style='padding:2rem;font-size:1.3rem;color:#888;'>No hay pedidos pendientes.</div>";
      }
      $conn->close();
      ?>
    </div>
  ...
  <div class="botonera">
    ...
    <button onclick="window.location.href='Cocina-Enproceso.php'">PEDIDOS EN PROCESO</button>
